...
def get_reward_sum(Q, V, state_a, state_b, whose_ball, action_num):
  k = 0
  val = 0
  i = 0
  for q in Q[state_a][state_b][whose_ball]:
    for j in q:
      val += j * V[k]
      k+=1
  return val
    ...
    for t in tqdm(range(num_steps)):
      ...
      if done:
        ...
        if vA['x'] != None and vB['x'] != None:
          v_sum_reward_a = get_reward_sum(Qa, vA['x'], state_a, state_b, whose_ball, num_action)
          v_sum_reward_b = get_reward_sum(Qb, vB['x'], state_a, state_b, whose_ball, num_action)
          v_sum_reward = (v_sum_reward_a + v_sum_reward_b) * np.array(vA['x']).max()

          a_reward = (1-gamma)* a_reward + gamma * v_sum_reward
          b_reward = (1-gamma)* b_reward + gamma * v_sum_reward
...
if __name__ == '__main__':
    ...
    plot_errors("Q-learner", "Simulation Iteration", "Q-Value Difference", Qa_episode_error_data, Qa_episode_error_steps)
